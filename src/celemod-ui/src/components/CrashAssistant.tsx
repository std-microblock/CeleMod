...
const readSeenCrashes = () => {
  try {
    return JSON.parse(localStorage.getItem(SEEN_CRASHES_KEY) || '{}') as Record<string, SeenCrash>;
  } catch {
    return {};
  }
};
...
const hasSeenCrash = (gamePath: string, analysis: CrashAnalysis) => {
  ...
  if (seen.eventId && analysis.eventId && seen.eventId === analysis.eventId) return true;
  ...
  return seen.crashIndex === analysis.crashIndex
    && seen.sourceLog?.toLocaleLowerCase() === analysis.sourceLog.toLocaleLowerCase()
...
const findCrashModFix = async (analysis: CrashAnalysis): Promise<CrashModFix | null> => {
  const updateInfo = await getLatestUpdateInfo();
  const crashText = `${analysis.exception}\n${analysis.excerpt}`.toLocaleLowerCase();
  return (updateInfo.crash_mod_fixes || []).find((fix) => {
    const suspect = analysis.suspects.find(
      (item) => item.name.toLocaleLowerCase() === fix.mod_name.toLocaleLowerCase(),
    );
    const affectedVersion = suspect
      ? fix.affected_versions.includes(suspect.installedVersion)
      : fix.affected_versions.some(
        (version) => crashText.includes(`${fix.mod_name} ${version}`.toLocaleLowerCase()),
      );
    return Boolean(
      affectedVersion
      && (fix.match?.contains || []).every(
        (part) => crashText.includes(part.toLocaleLowerCase()),
      ),
    );
  }) || null;
};

const loadLatestEverest = async (analysis: CrashAnalysis): Promise<EverestUpdate | null> => {
  if (analysis.isEverestUltra) {
    const updateInfo = await getLatestUpdateInfo();
    const latest = (updateInfo.everest_ultra?.versions || [])
      .filter((version) => !version.channel || version.channel.toLocaleLowerCase() === 'stable')
      .sort((left, right) => versionBuild(right.version) - versionBuild(left.version))[0];
    return latest ? {
  ...
  const response = await fetch('https://maddie480.ovh/celeste/everest-versions?supportsNativeBuilds=true');
...
const fileName = (path: string) => path.split(/[\\/]/).pop() || 'CeleMod-Crash.txt';
...
const startReportDrag = (event: DragEvent<HTMLDivElement>, path: string) => {
  ...
  event.dataTransfer.setData('DownloadURL', `text/plain:${fileName(path)}:${uri}`);
  event.dataTransfer.setData('text/uri-list', uri);
  event.dataTransfer.setData('text/plain', path);
};

const cleanLogPrefix = (line: string) => line
  .replace(/^\([^)]*\)\s+\[Everest\]\s+\[[^\]]+\]\s+\[[^\]]+\]\s*(?:>>\s*)?/, '')
  .trim();
...
const formatStacktrace = (analysis: CrashAnalysis, exception: string) => {
  ...
  const markerIndex = lines.findLastIndex((line) => /critical error/i.test(line));
  const exceptionIndex = lines.findIndex((line, index) => (
    index >= Math.max(0, markerIndex)
    && /\b(?:System\.)?[\w.`+]+Exception(?::|\s)/.test(line)
  ));
  const start = exceptionIndex >= 0 ? exceptionIndex : Math.max(0, markerIndex + 1);
  const cleaned = lines.slice(start, start + 180)
    .map((line) => line
      .replace(/^\([^)]*\)\s+\[Everest\]\s+\[[^\]]+\]\s+\[[^\]]+\]\s*(?:>>\s*)?/, '')
      .replace(/^\s*--->\s*/, '↳ ')
      .replace(/^\s+(at|在)\s+/, '  $1 ')
      .trimEnd())
    .filter((line, index) => {
      if (index === 0 && cleanLogPrefix(line) === exception.trim()) return false;
      return !/ENCOUNTERED A CRITICAL ERROR/i.test(line);
    });
  return cleaned.join('\n').trim() || analysis.excerpt;
};

const CrashPopup = ({ analysis, gamePath }: { analysis: CrashAnalysis; gamePath: string }) => {
  ...
  const [selected, setSelected] = useState(() => new Set(analysis.suspects.map((suspect) => suspect.name)));
  const [latestEverest, setLatestEverest] = useState<EverestUpdate | null>(null);
  ...
  useEffect(() => {
    callRemote('get_mod_latest_info', (data: string) => {
      const latest = new Map(
        (JSON.parse(data) as [string, string, string, string][])
          .map(([name, version, fileId, url]) => [name.toLocaleLowerCase(), { version, fileId, url }]),
      );
      setSuspects((current) => current.map((suspect) => {
        ...
        return {
          ...
          updateAvailable: compareLooseVersion(update.version, suspect.installedVersion) > 0,
        };
      }));
    }).catch(console.error);
  }, []);
  ...
  const selectedUpdates = selectedSuspects.filter((suspect) => suspect.updateAvailable);
  ...
  const stacktrace = useMemo(() => formatStacktrace(analysis, exception), [analysis, exception]);
  ...
  const restart = (legacyLoader = false) => runAction(async () => {
    setStatus(legacyLoader ? _i18n.t('正在使用 Legacy Loader 重启…') : _i18n.t('正在重启…'));
    await callRemote('restart_game_with_loader', gamePath, legacyLoader);
  });

  const disableAndRestart = () => runAction(async () => {
    if (selectedSuspects.length === 0) throw new Error(_i18n.t('请至少选择一个 Mod'));
    setStatus(_i18n.t('正在禁用所选 Mod 并重启…'));
    await globalContext.blacklist.setModsEnabled(
      selectedSuspects.map((suspect) => ({ name: suspect.name, file: suspect.file })),
      false,
    );
  ...
  const updateSelectedMods = async () => {
    if (selectedUpdates.length === 0) throw new Error(_i18n.t('所选 Mod 没有可用更新'));
    ...
    for (let index = 0; index < selectedUpdates.length; index += 1) {
      ...
      setStatus(_i18n.t('正在更新 {name} ({current}/{total})…', {
        name: suspect.name,
        current: index + 1,
        total: selectedUpdates.length,
      }));
      const source = suspect.gameBananaFileId !== undefined && suspect.gameBananaFileId !== -1
        ? String(suspect.gameBananaFileId)
        : suspect.downloadUrl;
      if (!source) throw new Error(`${suspect.name}: ${_i18n.t('没有可用下载地址')}`);
      await new Promise<void>((resolve, reject) => {
        downloadMod(suspect.name, source, {
          force: true,
          ownerId: `crash-${analysis.fingerprint}`,
          onProgress: (_task, progress) => setStatus(
            `${_i18n.t('正在更新')} ${suspect.name} · ${progress.toFixed(0)}%`,
          ),
          onFinished: () => resolve(),
          onFailed: (_task, reason) => reject(new Error(`${suspect.name}: ${reason}`)),
        });
      });
    }
  };

  const updateModsAndRestart = () => runAction(async () => {
    await updateSelectedMods();
    setStatus(_i18n.t('更新完成，正在重启…'));
    await callRemote('restart_game_with_loader', gamePath, false);
  });

  const updateEverestAndRestart = () => runAction(async () => {
      ...
      callRemote(
        ...
        (state: string, data: unknown) => {
          if (state === 'Failed') reject(new Error(String(data)));
            ...
            const progress = typeof data === 'number' ? ` · ${data.toFixed(0)}%` : '';
  ...
  const installCrashModFixAndRestart = () => runAction(async () => {
  ...
  return (
    <div className="popup-content crash-assistant-popup">
      <div className="crash-popup-heading">
        <div className="crash-popup-icon"><Icon name="warn" /></div>
        <div>
          <div>{_i18n.t('检测到 Everest 崩溃')}</div>
          <div className="crash-time">
            {new Date(analysis.logModifiedAt).toLocaleString()} · #{analysis.crashIndex}
          </div>
        </div>
        <button className="crash-close" disabled={busy} onClick={close} aria-label={_i18n.t('关闭')}>×</button>
      </div>
      ...
      <div className="crash-main-grid">
        <section className="crash-suspects">
          ...
          {suspects.length === 0 ? (
            <div className="crash-empty">{_i18n.t('未能从 stacktrace 中可靠定位到具体 Mod')}</div>
          ) : suspects.map((suspect) => {
            const dependents = suspect.dependents || [];
            return <label className="crash-suspect" key={suspect.name}>
              <input
                type="checkbox"
              ...
              <span className="crash-suspect-main">
                <span><strong>{suspect.name}</strong><em>{suspect.confidence}%</em></span>
                <small>{suspect.installedVersion}{suspect.updateAvailable ? ` → ${suspect.latestVersion}` : ''}</small>
                <small title={suspect.evidence}>{suspect.evidence}</small>
                {dependents.length > 0 ? (
                  <small className="crash-dependent-list" title={dependents.map((item) => item.name).join(', ')}>
                    {_i18n.t('被这些 Mod 依赖')}: {dependents.map((item) => (
                      `${item.name}${item.optional ? ` (${_i18n.t('可选')})` : ''}`
                    )).join(', ')}
                  </small>
                ) : null}
              </span>
              {suspect.updateAvailable ? <span className="crash-update-badge">{_i18n.t('可更新')}</span> : null}
            </label>;
          })}
        </section>
        ...
        <section className="crash-trace">
          <div className="crash-trace-heading">
            ...
            <div className="crash-everest-state">
              ...
              {analysis.isEverestUltra ? <span className="ultra">Ultra</span> : null}
              {everestUpdateAvailable ? (
                <span className="outdated">{_i18n.t('最新版 {version}', { version: latestEverest!.displayVersion })}</span>
              ) : null}
            </div>
          </div>
      ...
      <div className="crash-advice">
        ...
        <div>{analysis.suggestions.map((suggestion) => <span key={suggestion}>{suggestion}</span>)}</div>
        {latestError ? <small>{_i18n.t('检查 Everest 更新失败')}: {latestError}</small> : null}
      </div>
      ...
      >
        ...
        <button onClick={() => callRemote('reveal_crash_report', analysis.reportPath)}>
          {_i18n.t('打开所在位置')}
        </button>
      </div>
      ...
      <div className="buttons crash-actions">
        {crashModFix ? (
          <button className="primary" disabled={busy} onClick={installCrashModFixAndRestart}>
            {_i18n.t('安装 {name} 修复并重启', { name: crashModFix.mod_name })}
          </button>
        ) : null}
        {suspects.length > 0 ? (
          <button disabled={busy || selectedSuspects.length === 0} onClick={disableAndRestart}>
            {_i18n.t('禁用所选 Mod 并重启')}
          </button>
        ) : null}
        {suspects.some((suspect) => suspect.updateAvailable) ? (
          <button className="primary" disabled={busy || selectedUpdates.length === 0} onClick={updateModsAndRestart}>
            {_i18n.t('更新所选 Mod 并重启')}
          </button>
        ) : null}
        {everestUpdateAvailable ? (
          <button className="primary" disabled={busy} onClick={updateEverestAndRestart}>
        ...
        <button disabled={busy} onClick={close}>{_i18n.t('忽略')}</button>
      </div>
    </div>
  );
};
...
export const CrashAssistant = () => {
  ...
  useEffect(() => {
    ...
    const check = async () => {
      ...
      checking = true;
      try {
        const analysis = await callRemote<CrashAnalysis | null>('check_everest_crash', gamePath);
