                    ...
                    <Button onClick={async () => {
                        ...
                        createPopup(() => {
                            ...
                            useEffect(() => {
                                ...
                                // @ts-ignore
                                for (const script of div.querySelectorAll('script, iframe, style, link, meta'))
                                    script.remove();
                                // @ts-ignore
                            ...
                            return <div className="mod-detail-popup">
                                <div className="closeBtn" onClick={() => ctx.hide()}>
                                    <Icon name="i-cross" />
                                </div>
                                {
                                    data.externalUrl && <div className="openExternal" onClick={() => {
                                ...
                                <h2>{mod.name}</h2>
                                <div className="info">
                                    Mod · {data.lastUpdate ? displayDate(data.lastUpdate) + ' ·' : ''}{mod.author}
                                </div>
                                ...
                                {
                                    data.images && <div className="images" ref={refImages}>
                                        {data.images.map(v => <img src={v + "?h=160"} alt="" srcset="" onClick={
                                            () => createPopup(() => <div className="image-view">
                ...
                {props.mods.map(mod2 => {
                    const mod = useMemo(() => {
                        const res = {
                            ...
                            downloadUrl: () => {
                                const dedup = new Set()
                                return Promise.resolve(mod2.files.filter(v => {
