...
const status: SteamStatus = {
  ...
  totalBytes: 8912896,
  transferredBytes: 475136,
  cachedFiles: 100,
  currentFile: "a-very-long-mod-save-file.celeste",
  message: "正在传输 a-very-long-mod-save-file.celeste",
};
...
test("sync speed uses network bytes, not cached saves or file counts", () => {
  ...
  meter.sample(
    { ...status, done: 1, completedBytes: 1024, transferredBytes: 512 },
    0,
  );
  const rate = meter.sample(
    { ...status, done: 200, completedBytes: 2048, transferredBytes: 1536 },
    2000,
  );
  assert.equal(rate.bytesPerSecond, 512);
  assert.equal(rate.secondsRemaining, (status.totalBytes! - 2048) / 512);
  assert.deepEqual(
    meter.sample({ ...status, busy: false, stage: "error" }, 3000),
    {},
  );
});
test("unknown sync totals stay indeterminate and units do not contain line breaks", () => {
  const html = renderToStaticMarkup(
    <SteamSyncProgress status={{ ...status, total: 0 }} compact />,
  );
