...
export type SteamDownloadRate = { bytesPerSecond?: number; secondsRemaining?: number };
type Sample = { at: number; done: number; downloaded: number; transferred: number };
...
export class SteamDownloadMeter {
  ...
  sample(status: SteamStatus | undefined, now: number): SteamDownloadRate {
    ...
    while (this.samples.length > 2 && this.samples[1].at <= now - 8000) this.samples.shift();
    ...
    return {
      ...
      secondsRemaining: point.done >= total! ? 0 : usefulRate > 0 ? (total! - point.done) / usefulRate : undefined,
    };
  }
}

export function steamSpeed(rate?: number) {
  if (rate === undefined || !Number.isFinite(rate) || rate < 0) return "计算速度…";
  if (rate === 0) return "0 B/s";
  const units = ["B/s", "KiB/s", "MiB/s", "GiB/s"];
  const unit = Math.min(3, Math.max(0, Math.floor(Math.log(rate) / Math.log(1024))));
