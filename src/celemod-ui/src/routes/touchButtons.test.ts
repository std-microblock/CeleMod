...
import { actionKey, supportsTouch, toggleTouchAction, touchButtonError, TOUCH_ICONS } from "./touchButtons";
import { renderToStaticMarkup } from "react-dom/server";
import { createElement } from "react";
import { TouchButtonIcon } from "./TouchButtonIcon";

const chat = { source: "MiaoNet", actionPath: "/ChatButton", format: "standard" };
...
test("supported actions do not require physical key bindings", () => {
  ...
  assert.ok(supportsTouch({ source: "Celeste", actionPath: "/Jump", format: "vanilla" }));
  assert.ok(!supportsTouch({ ...chat, format: "legacyChord" }));
  assert.ok(!supportsTouch({ source: "Celeste", actionPath: "/Left", format: "vanilla" }));
});
test("button validation rejects empty labels/actions and excessive groups", () => {
  ...
  for (const change of [{ label: " " }, { label: "a".repeat(17) }, { label: "bad\nname" }, { actions: [] }, { actions: Array(17).fill(chat) }])
...
test("all selectable icons render a preview, with backwards-compatible text fallback", () => {
  for (const [icon] of TOUCH_ICONS) {
    const html = renderToStaticMarkup(createElement(TouchButtonIcon, { icon, label: "Test" }));
    assert.ok(html.includes(icon === "NONE" ? "Test" : "<svg"), icon);
  }
  assert.ok(renderToStaticMarkup(createElement(TouchButtonIcon, { label: "Old" })).includes("Old"));
});
