...
export const Loenn = () => {
  ...
  const refreshLocalState = () => callRemote<LoennState>('get_loenn_state')
    .then(setLocalState)
  ...
  const version = useMemo(
    () => config?.versions?.find((item) => item.version === selectedVersion) || config?.versions?.[0],
    [config, selectedVersion],
  );
  const selectedPackage: LoennPackage | undefined = platform && version
    ? version.packages[platform]
    : undefined;
  const installing = installState !== null && installState !== 'success' && installState !== 'failed';
  ...
  if (!updateInfo && !configError) {
    return <div className="loenn loenn-loading"><ProgressIndicator infinite /></div>;
  }

  if (!visible || !config) {
    return <div className="loenn loenn-unavailable">{_i18n.t('Loenn 当前不可用')}</div>;
  }
    ...
    <div className="loenn">
      ...
      <section className="loenn-local">
        <div>
          ...
          <strong>{localState?.installed ? localState.version : _i18n.t('尚未安装')}</strong>
          {localState?.installed ? <span className="installed"><Icon name="i-tick" /> {_i18n.t('已安装')}</span> : null}
        </div>
        <Button type="primary" disabled={!localState?.installed} onClick={launch}>
          {_i18n.t('启动 Loenn')}
        </Button>
      </section>
      ...
      <section className="loenn-install">
        <div className="install-heading">
          ...
          <label>
            ...
            <select value={version?.version || ''} onChange={(event) => setSelectedVersion(event.target.value)} disabled={installing}>
        ...
        {version ? (
          <div className="release-info">
            <span><Icon name="calendar" /> {displayDate(version.date)}</span>
            {selectedPackage?.size ? <span><Icon name="download" /> {formatSize(selectedPackage.size)}</span> : null}
        ...
        {!platform ? (
          ...
          <div className="package-note error">{_i18n.t('当前系统没有可用安装包')}</div>
        ) : null}

        {installing ? (
          <div className="loenn-progress">
            <ProgressIndicator value={installProgress || 0} max={100} size={38} lineWidth={3} />
          ...
          <div className="install-result failed">
            ...
            <Button onClick={() => setInstallState(null)}>{_i18n.t('返回')}</Button>
          </div>
        ) : installState === 'success' ? (
          <div className="install-result success">
            <Icon name="i-tick" />
            <strong>{_i18n.t('安装成功')}</strong>
            <Button type="primary" onClick={launch}>{_i18n.t('启动 Loenn')}</Button>
            <Button onClick={() => setInstallState(null)}>{_i18n.t('安装其他版本')}</Button>
          </div>
        ) : (
          <div className="install-actions">
            <Button type="primary" disabled={!selectedPackage} onClick={install}>
              <Icon name="download" /> {localState?.installed ? _i18n.t('安装此版本') : _i18n.t('安装 Loenn')}
            </Button>
          </div>
        )}
      </section>
