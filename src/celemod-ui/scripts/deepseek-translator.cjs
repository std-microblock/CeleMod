...
if (!API_KEY) {
  throw new Error('DEEPSEEK_API_KEY is required. Set it before running pnpm translate.');
}
  ...
  while (current.parent) {
    ...
    if (ts.isCallExpression(parent) && parent.arguments[0] === current) {
      ...
      if (
        ...
        (ts.isElementAccessExpression(expression) && expression.argumentExpression?.getText(sourceFile) === "'t'")
      ) {
        return 'i18n call';
      }
    }
    ...
    if (ts.isJsxText(parent) || ts.isJsxElement(parent) || ts.isJsxFragment(parent)) {
      return 'JSX content';
    }
  ...
  for (const filePath of walkSourceFiles(sourceRoot)) {
    ...
    function visit(node) {
      ...
      if (text) {
        ...
        let current = node.parent;
        while (current && !ts.isStatement(current) && !ts.isJsxElement(current)) {
          snippetNode = current;
          current = current.parent;
        }
...
function publicContexts(contexts) {
  return (contexts || []).map(({ signature: _signature, ...context }) => context);
}
...
function assertProtectedTokens(source, translation) {
  ...
  for (const [token, count] of expected) {
    if (actual.get(token) !== count) {
      throw new Error(`DeepSeek translation changed protected token ${JSON.stringify(token)}`);
    }
  }
  for (const [token, count] of actual) {
    if (expected.get(token) !== count) {
      throw new Error(`DeepSeek translation added protected token ${JSON.stringify(token)}`);
    }
  }
}
...
function buildDeepSeekRequest({ model, text, targetLang, contexts, correction }) {
...
async function requestDeepSeek({ apiKey, baseUrl, model, timeoutMs, text, targetLang, contexts, correction }) {
  ...
  try {
    const response = await fetch(`${normalizeBaseUrl(baseUrl)}/chat/completions`, {
      ...
      body: JSON.stringify(buildDeepSeekRequest({ model, text, targetLang, contexts, correction })),
    ...
    if (typeof result.translation !== 'string' || !result.translation.trim()) {
      throw new Error('DeepSeek JSON response has no non-empty translation field');
    }
    return result.translation;
  } catch (error) {
    if (error.name === 'AbortError') {
      const timeoutError = new Error(`DeepSeek API timed out after ${timeoutMs} ms`);
...
const concurrency = parsePositiveInteger(process.env.DEEPSEEK_CONCURRENCY, DEFAULT_CONCURRENCY);
...
async function translateByDeepSeek(text, targetLang) {
  ...
  const task = withSemaphore(async () => {
    const options = {
      ...
      timeoutMs: parsePositiveInteger(process.env.DEEPSEEK_TIMEOUT_MS, DEFAULT_TIMEOUT_MS),
      text,
      targetLang,
