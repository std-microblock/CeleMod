...
pub fn game_running() -> anyhow::Result<bool> {
    ...
    result["running"]
        .as_bool()
        .ok_or_else(|| anyhow::anyhow!("Android game state is unavailable"))
}
...
pub fn settings(
    ...
    Ok(if joystick.is_some() || game_rumble.is_some() {
        handle.run_mobile_plugin(
            "configure",
            serde_json::json!({"joystick": joystick, "gameRumble": game_rumble}),
        )?
    } else {
        handle.run_mobile_plugin("info", ())?
    })
}

pub fn init() -> TauriPlugin<Wry> {
    Builder::new("celeste-runtime")
        .setup(|_, api| {
            ...
            if let Some(root) = info["gameRoot"].as_str() {
                let _ = GAME_ROOT.set(root.into());
            }
...
pub fn game_dirs() -> Vec<PathBuf> {
    GAME_ROOT
        ...
        .filter(|p| p.join("Celeste.exe").is_file() || p.join("Celeste.dll").is_file())
        .collect()
}
...
pub fn launch(path: &str, origin: bool, legacy_loader: bool) -> anyhow::Result<()> {
    let _: serde_json::Value = RUNTIME
        ...
        .run_mobile_plugin(
            "launch",
            serde_json::json!({"path": path, "origin": origin, "legacyLoader": legacy_loader}),
        )?;
    Ok(())
}

pub fn install_everest(
    path: &Path,
    step: &str,
    progress: &mut dyn FnMut(String, f32),
) -> anyhow::Result<()> {
    use std::{
        sync::mpsc,
    ...
    let logs = GAME_ROOT
        .get()
        .and_then(|p| p.parent())
        .ok_or_else(|| anyhow::anyhow!("Missing Android game root"))?
        .join("logs");
    ...
    let result = std::thread::scope(|scope| {
        ...
        scope.spawn(move || {
            let result = runtime.run_mobile_plugin::<serde_json::Value>(
                "installEverest",
        ...
        loop {
            ...
            if std::fs::read_to_string(logs.join("installer-session"))
                .ok()
                .as_deref()
                == Some(request_id.as_str())
            {
                if let Ok(snapshot) = crate::log_viewer::read(&logs.join("installer.log")) {
            ...
            match result {
                ...
                Err(mpsc::RecvTimeoutError::Disconnected) => {
                    break Err(anyhow::anyhow!("Android installer worker disconnected"));
                }
                Err(mpsc::RecvTimeoutError::Timeout) => progress(
                    format!("{step}: {}", state.status(&log, start.elapsed().as_secs())),
                    -1.0,
                ),
            }
        }
