...
1. 从 Android 主页或设置页的 Steam 入口登录。默认保持登录，重新打开应用直接使用已保存会话；支持手机确认、手机验证码和邮件验证码，手机确认时可以改用验证码。
...
5. 冲突时明确选择手机版或云端版；两边旧版本先备份。退出账号删除本地保存的账号、密码和令牌，游戏、备份和未同步存档保留。令牌过期后可使用已保存密码重新验证，Steam 要求时仍需完成 Guard。

诊断命令 `probe` 只进行匿名连接和 Celeste 产品信息读取，不需要密码，不读取/修改云存档；不再放进登录界面。

## 数据安全与恢复

- SteamKit2 在现有 CoreCLR 中运行；Tauri/Kotlin 只传递命令、状态和结果。没有 localhost 网络服务、第三方登录网页或开发者 Web API key。
- 成功登录后，账号、密码、refresh token、Steam 设备 client ID 自动使用 Android Keystore AES-GCM 加密保存。日常下载和同步复用 token；重新验证时由原生层读取已保存密码，WebView 只收到 `hasSavedPassword`，不会收到已保存密码或 token。密码只允许当前匹配账号复用，切换账号必须重新输入；失败/取消不会覆盖先前验证成功的凭据。IPC 在应用私有目录，消费后删除，冷启动清理遗留请求/结果，但保留加密凭据。Android 备份在 manifest 中禁用。旧版本的 token-only 凭据继续可用，下次成功密码登录时补存密码。
...
- 主页和设置页只显示一张 Steam 状态入口卡。有持久会话时直接进入游戏概览，即使上次登录切换失败/取消也不丢失原会话。首次登录页只显示账号、密码和登录按钮，不展示登录/安全/兼容性说明或“记住我”开关。关闭会清空未提交的输入并归还焦点，不清除原生层已保存凭据，也不会取消后台任务。
- 登录、手机确认、邮件/令牌验证码、下载、云存档冲突分别显示独立步骤。验证码自动规整为大写，提交后避免重复发送；每次 Guard 请求带独立状态 revision，连续输错也能重新输入。
- 默认手机确认；可改选验证码，不提供假的扫码登录入口。下载、关闭自动云存档、退出账号和解决冲突使用面板内的确认流程，不弹浏览器 confirm。
